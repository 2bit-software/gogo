...
func buildBinary(optimize bool, sourceDir, to string) error {
	// go mod tidy
	err := cmdr.New("go", "mod", "tidy").Dir(sourceDir).Run()
	if err != nil {
		return fmt.Errorf("failed to tidy go modules: %w", err)
	}

	formatOutput, err := cmdr.New("go", "fmt", ".").Dir(sourceDir).String()
	if err != nil {
		return fmt.Errorf("failed to format rendered document: `%v` due to %w", formatOutput, err)
	}

	// go get
	err = cmdr.New("go", "get").Dir(sourceDir).Run()
	if err != nil {
	...
	// build
	out, err := cmdr.New(cmd...).Dir(sourceDir).String()
	if err != nil {
		_ = os.Remove(to)
